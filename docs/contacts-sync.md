...
1. **Get the sync secret.** It was generated and set on the Worker already; the value is in your local
   `.dev.vars` as `CONTACTS_SYNC_SECRET` (gitignored). Copy it. (To rotate it later:
   `wrangler secret put CONTACTS_SYNC_SECRET` and update the Apps Script property to match.)
...
4. **Set Script Properties.** **Project Settings** (gear) → **Script Properties** → add:
   - `SCREENER_URL` = `https://ai-call-screener.will-c-armstrong.workers.dev/sync-contacts`
